...
//                    PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_MUTABLE);
          alarmIntent =  PendingIntent.getBroadcast(mContext, alarmId, intent, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_MUTABLE);
        }else {
    ...
    void stopAlarm(AlarmModel alarm) {
        ...
        Intent intent = new Intent(mContext, AlarmReceiver.class);
       // PendingIntent alarmIntent = PendingIntent.getBroadcast(mContext, alarmId, intent,   (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S)? PendingIntent.FLAG_MUTABLE : PendingIntent.FLAG_UPDATE_CURRENT);
        PendingIntent alarmIntent = null;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
           alarmIntent = PendingIntent.getBroadcast(mContext, alarmId, intent, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_MUTABLE);
        }else {
    ...
    private PendingIntent createOnDismissedIntent(Context context, int notificationId) {
        ...
        PendingIntent alarmIntent = null;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
          alarmIntent =  PendingIntent.getBroadcast(context.getApplicationContext(), notificationId, intent, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_MUTABLE);
        }else {
            alarmIntent = PendingIntent.getBroadcast(context.getApplicationContext(), notificationId, intent, 0);
        }
    ...
    void sendNotification(AlarmModel alarm) {
        try {
           ...
           // PendingIntent pendingIntent = PendingIntent.getActivity(mContext, notificationID, intent, PendingIntent.FLAG_UPDATE_CURRENT);
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
              pendingIntent =  PendingIntent.getBroadcast(mContext, notificationID, intent, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_MUTABLE);
            }else {
                pendingIntent = PendingIntent.getBroadcast(mContext, notificationID, intent, 0);
            }
            ...
            if (alarm.isHasButton()) {
                ...
                PendingIntent pendingDismiss = PendingIntent.getBroadcast(mContext, notificationID, dismissIntent,  (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S)? PendingIntent.FLAG_MUTABLE : PendingIntent.FLAG_UPDATE_CURRENT);
                ...
                PendingIntent pendingSnooze = PendingIntent.getBroadcast(mContext, notificationID, snoozeIntent, (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S)? PendingIntent.FLAG_MUTABLE : PendingIntent.FLAG_UPDATE_CURRENT);
